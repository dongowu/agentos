...
cargo run -- team-run "交付一个用户登录系统" --llm-adapter script --llm-model gpt-4.1 --llm-script-command "printf '%s (%s)' \"$ORCH_TASK_TITLE\" \"$ORCH_MODEL\"" --llm-script-max-attempts 2 --llm-script-timeout-ms 30000
...
- 脚本超时：`--llm-script-timeout-ms`（默认无超时，仅 script adapter 生效）
...
- 表达式条件：`condition_expression` 支持 `risk==...`、`risk>=...`、`risk<=...`、`retry==...`、`retry>=...`、`retry<=...`、`retry>...`、`retry<...`、`team_load==...`、`team_load<=...`、`team_load>=...`、`team_load>...`、`team_load<...`、`true`/`false`，并可用 `&&`/`||`/`!`/`(...)`
- 运算符左右允许空格（例如 `retry >= 1 && risk == low`）
